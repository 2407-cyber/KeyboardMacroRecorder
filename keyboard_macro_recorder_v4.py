...
class App:
    ...
    def _build_ui(self):
        # 紧凑、清爽的 Windows 风格界面；布局按最终效果图重做。
        outer = tk.Frame(self.root, bg="#ffffff")
        outer.pack(fill="both", expand=True, padx=18, pady=14)

        # 标题区
        title = tk.Frame(outer, bg="#ffffff", height=48)
        title.pack(fill="x")
        title.pack_propagate(False)
        tk.Label(title, text="◉  键盘宏 v4", bg="#ffffff", fg="#111111",
                 font=self._font(21, True)).pack(side="left", padx=2)
        tk.Label(title, textvariable=self.status, bg="#ffffff", fg="#111111",
                 font=self._font(11)).pack(side="right", padx=4)

        # 主操作按钮
        actions = tk.Frame(outer, bg="#ffffff")
        actions.pack(fill="x", pady=(6, 14))
        buttons = [
            ("●  开始录制", self.toggle_record),
            ("▶  播放", self.start_play),
            ("■  停止", self.stop_play),
            ("⌫  清空", self.clear),
        ...
        for i, (text, cmd) in enumerate(buttons):
            b = self._big_button(actions, text, cmd)
            b.grid(row=0, column=i, sticky="ew", padx=(0 if i == 0 else 6, 0 if i == 4 else 6))
            actions.grid_columnconfigure(i, weight=1)
            if i == 0: self.rec_btn = b
            elif i == 1: self.play_btn = b
            elif i == 2: self.stop_btn = b
            elif i == 3: self.clear_btn = b
            else: self.settings_btn = b

        # 配置区
        cfg = tk.Frame(outer, bg="#ffffff")
        cfg.pack(fill="x", pady=(0, 14))
        tk.Label(cfg, text="当前快捷键配置：", bg="#ffffff", fg="#111111",
                 font=self._font(15)).pack(side="left", padx=(4, 12))
        self.config_combo = ttk.Combobox(cfg, textvariable=self.current_config,
                                         state="readonly", font=self._font(12), height=10)
        self.config_combo.pack(side="left", fill="x", expand=True, ipady=5)
        self.config_combo.bind("<<ComboboxSelected>>", lambda e: self._switch_config())
        for text, cmd in (("＋  新建", self.new_config), ("✎  重命名", self.rename_config), ("▣  删除", self.delete_config)):
            self._medium_button(cfg, text, cmd).pack(side="left", padx=(8, 0), ipadx=9, ipady=5)

        # 动作列表
        list_box = tk.Frame(outer, bg="#ffffff", highlightbackground="#d8dee8", highlightthickness=1)
        list_box.pack(fill="both", expand=True, pady=(0, 14))
        self.list_title = tk.Label(list_box, text="动作列表（共 0 个动作）", bg="#ffffff", fg="#111111",
                                   font=self._font(15), anchor="w")
        self.list_title.pack(fill="x", padx=14, pady=(11, 7))
        ...
        headings = [
            ("no", "序号", 60), ("action", "动作", 105), ("key", "按键", 145),
            ("state", "按键状态", 130), ("wait", "与上一动作间隔 (ms)", 210),
            ("total", "累计时间 (ms)", 160)
        ]
        for col, text, width in headings:
            self.tree.heading(col, text=text)
            self.tree.column(col, width=width, minwidth=55, anchor="center", stretch=True)
        ...
        # 底部参数
        bottom = tk.Frame(outer, bg="#ffffff", highlightbackground="#d8dee8", highlightthickness=1)
        bottom.pack(fill="x", pady=(0, 8))
        fields = tk.Frame(bottom, bg="#ffffff")
        fields.pack(fill="x", padx=10, pady=11)
        ...
        # 页脚
        footer = tk.Frame(outer, bg="#ffffff", height=22)
        footer.pack(fill="x")
        footer.pack_propagate(False)
        tk.Label(footer, text="提示：按 F8 开始/停止播放，按 F9 停止播放",
                 bg="#ffffff", fg="#333333", font=self._font(9)).pack(side="left")
        self.footer_count = tk.Label(footer, text="动作数：0        版本：v4.0.0",
                 bg="#ffffff", fg="#333333", font=self._font(9))
        self.footer_count.pack(side="right")

        style = ttk.Style()
        try: style.theme_use("clam")
        except Exception: pass
        style.configure("Treeview", font=self._font(10), rowheight=32,
                        background="#ffffff", fieldbackground="#ffffff", foreground="#111111",
                        borderwidth=0)
        style.configure("Treeview.Heading", font=self._font(10, True),
                        background="#f4f7fb", foreground="#111111", relief="flat", padding=6)
        style.map("Treeview", background=[("selected", "#dcecff")], foreground=[("selected", "#111111")])
        style.configure("TCombobox", font=self._font(12), padding=5)
        style.configure("TEntry", font=self._font(11), padding=4)

    def _big_button(self, parent, text, command):
        return tk.Button(parent, text=text, command=command, font=self._font(14),
                         bg="#ffffff", fg="#111111", activebackground="#edf4ff",
                         activeforeground="#111111", relief="solid", bd=1,
                         highlightthickness=0, cursor="hand2", padx=5, pady=9)

    def _medium_button(self, parent, text, command):
        return tk.Button(parent, text=text, command=command, font=self._font(11),
                         bg="#ffffff", fg="#111111", activebackground="#edf4ff",
                         activeforeground="#111111", relief="solid", bd=1,
    ...
    def _field(self, parent, label, variable, index, suffix=""):
        ...
        box.grid(row=0, column=index, sticky="ew", padx=8)
        parent.grid_columnconfigure(index, weight=1)
        tk.Label(box, text=label, bg="#ffffff", fg="#111111", font=self._font(10)).pack(anchor="w", pady=(0, 4))
        row = tk.Frame(box, bg="#ffffff")
        row.pack(fill="x")
        ent = ttk.Entry(row, textvariable=variable, font=self._font(11), width=10)
        ent.pack(side="left", fill="x", expand=True, ipady=4)
        if suffix:
            tk.Label(row, text=suffix, bg="#ffffff", fg="#111111", font=self._font(10)).pack(side="left", padx=(6,0))
    ...
    def refresh(self):
        ...
        if hasattr(self, "footer_count"):
            self.footer_count.config(text=f"动作数：{len(self.events)}        版本：v4.0.0")
